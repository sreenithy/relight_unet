...
class RelightNetwork(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        wl2 = 0.08*torch.sum(weights * torch.abs(light_estim- light_input))**2 / face_estim.shape[0]
        ...
        if self.log_images:
            if self.global_step % 10 == 0:
                ...
                img_stack = torch.clamp(
                    torch.cat((input_[0:1, ...]*0.4545, output_face[0:1, ...]*0.4545, face_diff*0.4545, face_estim[0:1, ...]*0.4545,
    ...
    def __dataloader(self):
        dataset_train = LightStageFrames(Path("ts/"))
        dataset_val = LightStageFrames(Path("tests/"))
